...
use syn::{
    FnArg, GenericArgument, Generics, ItemFn, Path, PathArguments, PathSegment, ReturnType,
...
fn parse_fn(fun: &ItemFn) -> Result<ImplInfo, TokenStream> {
    ...
    let (struct_path, data_type, update_type) =
        match fun.sig.inputs.iter().collect::<Vec<_>>().as_slice() {
            [this, data] => {
...
fn get_err_type(ret: &ReturnType) -> Result<&Type, TokenStream> {
    match ret {
        ReturnType::Type(_, t) => match t.as_ref() {
            Type::Path(p) => {
                let segment =
                    expect_1_path_segment(&p.path, "Expected ident in return type, found path.")?;
...
fn parse_return_type(segment: &PathSegment) -> Result<&Type, TokenStream> {
    if segment.ident != "Result" {
        return Err(compile_error(format!(
            "Expected `Result` type in return type, found {}",
            segment.ident
        )));
    }
...
fn validate_ok_type(ok: &Type) -> Result<(), TokenStream> {
    match ok {
        Type::Tuple(tup) => match tup.elems.len() {
            0 => Ok(()),
            count => {
                return Err(compile_error(format!(
                    "Expected tuple with no args, found {} args",
                    count
                )))
            }
        },
...
) -> Result<(&Type, &Type), TokenStream> 
{
    match segment.ident == "DataWithUWC" {
        true => match &segment.arguments {
            ...
            _ => Err(compile_error(
                "In second argument expected ident with 2 generics.",
            )),
        },
        false => Err(compile_error(format!(
            "In second argument expected DataWithUWC, found {}.",
            segment.ident
        ))),
    }
}
...
) -> Result<(&'a Type, &'a Type), TokenStream> 
{
    match generics {
        ...
        _ => Err(compile_error(format!(
            "In second argument expected 2 generics, found {}.",
            generics.len()
        ))),
    }
}
...
fn expect_1_path_segment<'a>(
    path: &'a Path,
    err: &'static str,
) -> Result<&'a PathSegment, TokenStream> {
